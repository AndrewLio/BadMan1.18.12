...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
	...
	if (strcmp(screenName.c_str(), "inventory_screen") == 0 || strcmp(screenName.c_str(), "small_chest_screen") == 0 || strcmp(screenName.c_str(), "large_chest_screen") == 0 || strcmp(screenName.c_str(), "ender_chest_screen") == 0 || strcmp(screenName.c_str(), "shulker_box_screen") == 0) {
		static auto killauraMod = moduleMgr->getModule<Speed>();
		static auto aidMod = moduleMgr->getModule<Killaura>();
		static auto invMod = moduleMgr->getModule<bedaura>();
		string le = "Disable BedAura";
		string sp = "Disable Speed";
		string ka = "Disable KillAura";
		string ba = "Disable BedAura";

		float buttonLen = DrawUtils::getTextWidth(&le, 1) + 10;
		...
		DrawUtils::drawText(textPos, &sp, MC_Color(255, 255, 255), 1, 1);
		DrawUtils::drawText(textPos2, &ka, MC_Color(255, 255, 255), 1, 1);
		DrawUtils::drawText(textPos3, &ba, MC_Color(255, 255, 255), 1, 1);
	}
	/*/
...

	{
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			...
			auto red = string(RED);
			auto blue = string(BLUE);
			auto green = string(GREEN);
			string Credits = (
				green + bold + string("Credits:\n") + blue +
				string("    Scaret446\n") + red +
				string("    ReaQwQ")
			);
			float wtextWid = DrawUtils::getTextWidth(&string("BadMan"), 2.f);
			DrawUtils::drawText(vec2_t(5, 5), &Credits, MC_Color(255, 255, 255), 1, 1, true);
			auto interfaceColor = ColorUtil::interfaceColor(1);
			DrawUtils::drawText(vec2_t(wid.x - wtextWid - 5, wid.y - wid.y + 7), &string("BadMan"), MC_Color(interfaceColor), 2.f, 1.f, true);
...
void Hooks::LoopbackPacketSender_sendToServer(C_LoopbackPacketSender* a, Packet* packet) {
	static auto oFunc = g_Hooks.LoopbackPacketSender_sendToServerHook->GetFastcall<void, C_LoopbackPacketSender*, Packet*>();
...
void Hooks::chatLogHookFunc(__int64 a1, TextHolder* msg, uint32_t a2) {
	...
	static auto backmod = moduleMgr->getModule<Interface>();
	...
	if (message.getText() != nullptr && player != nullptr) {
		...
		if (backmod->isEnabled()) {
			if (textStr.find("home") != string::npos || textStr.find("teleport") != string::npos || textStr.find("homes") != string::npos) {
				backmod->SendMsg = true;
				backmod->Myhome = textStr;
			}
			if (textStr.find("joining the chaos") != string::npos || textStr.find("drama") != string::npos || textStr.find("feels like") != string::npos) {
				backmod->tk2 = true;
				backmod->org2 = false;
			}
			if (textStr.find("Welcome") != string::npos || textStr.find("left hell") != string::npos || textStr.find("Heaven") != string::npos) {
				backmod->tk2 = false;
				backmod->org2 = true;
			}
		}
