...
TargetHUD::TargetHUD() : IModule(0, Category::VISUAL, "Displays information about your target") {
	...
	registerFloatSetting("Range", &rangeTGH, rangeTGH, 3.f, 50.f);
...
void TargetHUD::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.canUseMoveKeys() && !clickGUI->hasOpenedGUI) {
		...
		if (!targetListEmpty) {
			...
			string position = u8"×ø±ê: " + to_string((int)floorf(pos->x)) + " " + to_string((int)floorf(pos->y)) + " " + to_string((int)floorf(pos->z));
			string distance = u8"¾àÀë: " + to_string((int)dist) + string(".") + to_string((int)(dist * 10) - ((int)dist * 10)) + "m";
			...
			if (!targetListEmpty) {
				...
				if (!(targetList[0]->damageTime > 1)) DrawUtils::fillRectangle(testRect, MC_Color(0, 0, 0, opacity), 0.5F);
				else DrawUtils::fillRectangle(testRect, MC_Color(255, 0, 0, opacity), 0.5F);
				auto cccc = ColorUtil::interfaceColor(1);
				if (outlinetgh) DrawUtils::drawRectangle(testRect, MC_Color(cccc), 0.5F);
				dbrr1 = targetLen;
			...
			for (auto& i : targetList) {
				...
				vec2_t distPos2 = vec2_t(testRect.x + 4, testRect.y + 25);
				vec2_t armorPos = vec2_t(testRect.x + 4, testRect.y + 25);
				vec2_t armorPos2 = vec2_t(testRect.z + 70, positionY);
		...
		{
			DrawUtils::fillRectangle(vec4_t(positionX, positionY, dbrr1 + positionX + 90, positionY + dbrr2 * 2.5f + 25), MC_Color(0, 0, 0, opacity), 0.5F);
