	...
	if (localPlayer != nullptr && GameData::canUseMoveKeys()) {
		...
		for (C_Entity* ent : itemESP) {
			if (ent->getEntityTypeId() == 64 && espMod->isItemESP) {
				...
				float size = fmax(0.8f, 3.f / g_Data.getClientInstance()->levelRenderer->getOrigin().dist(*ent->getPos()));

				float itemSize = size;
				float Textlen = DrawUtils::getTextWidth(&text, 1.5f);
				float TextHigh = DrawUtils::getFontHeight(1.5f);
				vec4_t recPos = vec4_t(textPos.x - 15  ,textPos.y ,textPos.x - 2 , textPos.y + 13);
				vec4_t recPos2 = vec4_t(textPos.x, textPos.y, textPos.x + Textlen ,textPos.y + TextHigh);

				// if (!moduleMgr->getModule<NotoSans>()->isEnabled()) {
				// textPos.y += 1.f * size;
				// }

				 DrawUtils::fillRectangle(recPos, MC_Color(20, 20, 20), 0.4f);
				 DrawUtils::fillRectangle(recPos2, MC_Color(20, 20, 20), 0.5f);
				DrawUtils::drawItem(stack, vec2_t(textPos.x - 15 , textPos.y), 1.f, itemSize, false);
				DrawUtils::drawText(textPos, &text, MC_Color(255,0,0),1.5f,1,true);
