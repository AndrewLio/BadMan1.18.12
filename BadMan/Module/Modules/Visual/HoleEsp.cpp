#include "HoleEsp.h"
bool bedrockConditionMetLastTick = false;
bool obsidianConditionMetLastTick = false;
bool bedrockConditionMetThisTick = false;
bool obsidianConditionMetThisTick = false;
bool addholecount = false;
bool removeholecount = false;
int holeob = 0;
int holebd = 0;
...
void drawBlock(const vec3_t& blockPos, float red, float green, float blue, float opacity, float lineOpacity, bool outline, float LineSize) {
	vec2_t fillBoxThing1 = DrawUtils::worldToScreen(vec3_t(blockPos).add(1.f, -0.01f, 1.f));
	...
	if (outline) {
		DrawUtils::setColor(red, green, blue, lineOpacity);
...
const char* HoleEsp::getRawModuleName() {
	return ("HoleESP");
}
					...
					for (int z = (int)pos->z - range; z < pos->z + range; z++) {
						for (int y = (int)pos->y - range; y < pos->y + range; y++) {
							...
							if (block != nullptr && block2 != nullptr && block3 != nullptr && block4 != nullptr) {
								...
								// ÉÅÉCÉìÇÃèàóù
								if (bedrock && !bedrockConditionMetLastTick) {if ((blockId2 == 7 && blockId3 == 7 && blockId4 == 7 && blockId5 == 7 && blockId6 == 7 && blockId == 0) && (aircheck == 0)) {drawBlock(blockPos, 0.f, 1.f, 0.f, Opacity, lineOpacity, outline, LineSize);}}
								if (obsidian && !obsidianConditionMetLastTick) {if ((blockId2 == 49 || blockId2 == 7) && (blockId3 == 49 || blockId3 == 7) && (blockId4 == 49 || blockId4 == 7) && (blockId5 == 49 || blockId5 == 7) && (blockId6 == 49 || blockId6 == 7) && (blockId == 0) && (aircheck == 0) && !((blockId2 == 7 && blockId3 == 7 && blockId4 == 7 && blockId5 == 7 && blockId6 == 7 && blockId == 0) && (aircheck == 0))) {drawBlock(blockPos, 1.f, 0.f, 0.f, Opacity, lineOpacity, outline, LineSize);}}
							}
						}
					}
				}
