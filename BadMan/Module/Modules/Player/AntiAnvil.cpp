...
const char* AntiAnvil::getModuleName() {
    std::string name = "AntiAnvil " + std::string(GRAY) + std::string(blockname);
    return name.c_str();
}
const char* AntiAnvil::getRawModuleName() {
    return ("AntiAnvil");
}
...
bool AntiAnvil::isFallingAnvilAbovePlayer(C_Player* plr) {
    ...
    for (int yOffset = 0; yOffset <= (int)height; yOffset++) {  // Use the height value from the slider
        vec3_ti blockposAbove1 = blockpos3;
        blockposAbove1.y += yOffset;

        C_Block* blockAbove1 = g_Data.getLocalPlayer()->region->getBlock(blockposAbove1);
