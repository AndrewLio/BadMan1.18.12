...
Sneak::Sneak() : IModule(0, Category::MOVEMENT, u8"×Ô¶¯ÏÂ¶×") {
...
const char* Sneak::getModuleName() {
	if (!silentlol) {
		name = std::string("AutoSneak ") + std::string(GRAY) + std::string("Normal");
	}
	else
	{
		name = std::string("AutoSneak ") + std::string(GRAY) + std::string("Silent");
	}
	return name.c_str();
}
