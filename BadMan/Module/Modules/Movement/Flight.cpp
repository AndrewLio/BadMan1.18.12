...
using namespace std;
Flight::Flight() : IModule(0, Category::MOVEMENT, u8"·ÉÐÐ") {
...
const char* Flight::getModuleName() {
	if (mode.getSelectedValue() == 0)
		name = string("Flight ") + string(GRAY) + string("[") + string(WHITE) + string("Vanila") + string(GRAY) + string("]");
	if (mode.getSelectedValue() == 1)
		name = string("Flight ") + string(GRAY) + string("[") + string(WHITE) + string("Airwalk") + string(GRAY) + string("]");
	return name.c_str();
}
...
void Flight::onEnable() {
	...
	if (damage) {
		level->playSound("game.player.hurt", *pos, 1, 1);
		player->animateHurt();
	}
...
void Flight::onTick(C_GameMode* gm) {
	...
	if (mode.getSelectedValue() == 6) {
		if (g_Data.canUseMoveKeys()) {
			...
			if (TimerUtil::hasTimedElapsed(50, false));
		}
	}
}
...
void Flight::onSendPacket(Packet* packet) {
	C_GameSettingsInput* input = g_Data.getClientInstance()->getGameSettingsInput();
	auto player = g_Data.getLocalPlayer();
	if (player == nullptr) return;
