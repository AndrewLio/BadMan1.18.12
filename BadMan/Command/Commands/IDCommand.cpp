	...
	if (args->at(1) == "pointing") {
		...
		if (pointing->hasEntity()) {
			...
			if (entity != nullptr) {
				...
				clientMessageF("[BadMan] Entity Name: %s", entityName.c_str());
				clientMessageF("[BadMan] Entity ID: %s", entityID.c_str());
			}
		} else {
			clientMessageF("[BadMan] Block Name: %s", blockName);
			clientMessageF("[BadMan] Block ID: %d", blockID);
		}
		return true;
		...
		if (stack->item != nullptr) {
			...
			g_Data.getGuiData()->displayClientMessageF("[BadMan] Item Name: %s", name);
			g_Data.getGuiData()->displayClientMessageF("[BadMan] Item ID: %lld", id);
		}
		return true;
	}
	return false;
