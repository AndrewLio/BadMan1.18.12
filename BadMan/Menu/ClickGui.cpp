	...
	*/
	...
	if (ourWindow->isInAnimation) {
		if (ourWindow->isExtended) {
			clickGUI->animation *= 0.895f;
			if (clickGUI->animation < 0.01f) {
				ourWindow->yOffset = 0;
	...
	if (ourWindow->isExtended) {
		...
		for (auto& mod : moduleList) {
			...
			vec2_t textPos = vec2_t(currentXOffset + textPadding , currentYOffset + textPadding );
			...
			if (allowRender) {
				...
				if (!ourWindow->isInAnimation && !isDragging && rectPos.contains(&mousePos)) {
					...
					DrawUtils::fillRectangleA(vec4_t(rectPos.x, rectPos.y, rectPos.z, rectPos.w), mod->isEnabled() ? MC_Color(interfaceColor) : MC_Color(60, 60, 60, 150));
					//DrawUtils::fillRectangle(rectPos2, MC_Color(interfaceColor), 1.f);
					//DrawUtils::fillRectangle(rectPos3, MC_Color(interfaceColor), 1.f);
					...
					DrawUtils::fillRectangleA(vec4_t(rectPos.x, rectPos.y, rectPos.z, rectPos.w), mod->isEnabled() ? MC_Color(interfaceColor) : MC_Color(0, 0, 0, 150));
				}
			}
			...
			if (allowRender) {
				...
				DrawUtils::drawText(textPos, &textStr, mod->isEnabled() ? MC_Color(255, 255, 255) : MC_Color(175, 175, 175), 1.f, 1.f, true);
				/*
				if (mod->getKeybind() != 0x0) {
				...
				if (settings->size() > 2 && allowRender) {
					...
					if (clickGUI->Crossline) {
						GuiUtils::drawCrossLine(vec2_t(
							...
							mod->isEnabled() ? MC_Color(255, 255, 255) : MC_Color(175, 175, 175), 0.25f, crossSize, !clickMod->isExtended);
					}
					...
					currentYOffset += textHeight + (textPadding * 1.25);
						...
						for (auto setting : *settings) {
							...
							case ValueType::BOOL_T: {
								...
								vec4_t selectableSurface = vec4_t(
									...
									rectPos.z + textHeight - textPadding,
	...
	// Draw Category Header
	{
		...
		// Extend Logic
		{
			if (rectPos.contains(&mousePos) && shouldToggleRightClick && !isDragging) {
				...
				if (ourWindow->isExtended && ourWindow->animation == 0) ourWindow->animation = 0.002f;
		...
		// Draw category header
		{
			...
			DrawUtils::fillRectangle(vec4_t(rectPos.x - 0.1, rectPos.y, rectPos.z + 0.1, rectPos.w), MC_Color(interfaceColor), 1.f);
			/*
