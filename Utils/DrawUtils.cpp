...
void DrawUtils::fillbox3d(vec3_t lower, vec3_t upper, float lineWidth, bool outline) {
	vec3_t diff;
	...
	vec2_t filler[4];
	//top
	filler[0] = DrawUtils::worldToScreen(vec3_t(lower.x + 1.f, lower.y, lower.z));
	filler[1] = DrawUtils::worldToScreen(vec3_t(lower.x, lower.y + 1.f, lower.z + 1.f));
	//bottom
	filler[2] = DrawUtils::worldToScreen(vec3_t(lower.x, lower.y, lower.z));
	filler[3] = DrawUtils::worldToScreen(vec3_t(lower.x, lower.y + 1.f, lower.z + 1.f));
	vec2_t filler2[4];
	//top
	filler2[0] = DrawUtils::worldToScreen(vec3_t(lower.x + 1.f, lower.y, lower.z));
	filler2[1] = DrawUtils::worldToScreen(vec3_t(lower.x, lower.y + 1.f, lower.z + 1.f));
	//bottom
	filler2[2] = DrawUtils::worldToScreen(vec3_t(lower.x + 1.f, lower.y, lower.z + 1.f));
	filler2[3] = DrawUtils::worldToScreen(vec3_t(lower.x + 1.f, lower.y + 1.f, lower.z));
	...
	if (!outline) {
		for (auto it = screenCords.begin(); it != screenCords.end(); it++) {
			...
			for (auto to : screenCords) {
				...
				if (shouldDraw)
					//fillRectangle(fillpos, MC_Color(), 0.3f);
					DrawUtils::drawQuad(filler[0], filler[1], filler[2], filler[3]);
				DrawUtils::drawQuad(filler2[0], filler2[1], filler2[2], filler2[3]);
	...
	for (auto& indice : indices) {
		...
		DrawUtils::fillRectangle(fillpos, MC_Color(), 0.3f);
